...
<c:forEach items="${reviews}" var="review">
   <div class="container-fluid text-left content"> <p >${review.text}</p>
       <p class="text-left"><strong>Author:</strong> ${review.gest} Mark:${review.mark} Date:${review.date}
           <c:forEach items="${myopinions}" var="myopinion">
               <c:choose>
                   <c:when test="${myopinion.id == review.id}">
                       <a href="/article/editopinion/${review.id}" type="button" class="btn btn-primary">Edit</a>
                   </c:when>
               </c:choose>
           </c:forEach>
           <c:forEach items="${myopinions}" var="myopinion">
           <c:choose>
           <c:when test="${myopinion.id == review.id || login==article.id }">
         <a href="/article/deleteopinion/${review.id}" type="button" class="btn btn-danger">Delete</a>
       </c:when>
       </c:choose>
       </c:forEach></div>
</c:forEach>
